...
pub const SPF_MIN_ADDR: u64 = 0x800_0100;
pub const SPF_FABS: u64 = 0x800_bc40 - 4;
pub const SPF_FCN: u64 = 0x800_0400 - 4;
...
impl JumpLinkHandler for SysProvided {
    fn should_handle(&self, new_pc: u64) -> bool {
        println!("{new_pc} >= {SPF_MIN_ADDR} => {}", new_pc >= SPF_MIN_ADDR);
        new_pc >= SPF_MIN_ADDR
    }
    fn handle(&self, new_pc: u64, cpu: &Cpu) -> (XRegisters, FRegisters) {
        println!("Executing {new_pc:x}");
...
fn main() -> Result<(), Exception> {
    ...
    let mut dram = Arc::new(Mutex::new(Dram::new(DRAM_SIZE)));
    dram.lock().unwrap().initialize(prog);
